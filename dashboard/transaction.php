<?php
    ...
    if (isset($_GET["formSearch"]) && isset($_GET["type"]) && isset($_GET["query"])) {
        ...
        $sql = "SELECT * FROM transaction LIMIT 100";

        if ($query != "") {
            $mappings = array("transaction ID"=>"id", "Full Name"=>"name", "Email"=>"email", "Address"=>"address");
            $type = $mappings[$type];
            $sql = "SELECT * FROM transaction WHERE $type LIKE '%$query%' LIMIT 100";
        }
        ...
        if (isset($_GET["formEnable"]) || isset($_GET["formDisable"])) {
            ...
            if (isset($_GET["formEnable"])) {
                $sql = "UPDATE transaction SET enabled = TRUE WHERE id IN($ids)";
            } else if (isset($_GET["formDisable"])) {
                $sql = "UPDATE transaction SET enabled = FALSE WHERE id IN($ids)";
            }
...
<html>
    ...
    <body>
        ...
        <nav>
            <ul>
                <li><a href="transaction.php">Client</a></li>
        ...
        <div id="container">
            <div id="top-bar">
                    ...
                    <div id="top-bar-toggles">
                        ...
                        <form id="formEnable" action="transaction.php" method="get">
                            <input id="btn-enable" type="submit" name="formEnable" value="Enable">
                            <input id="hiddenEnable" type="hidden" name="hidden">
                        </form>
                        <form id="formDisable" action="transaction.php" method="get">
